    ...
    <div class="bg-white p-4 md:p-6 rounded-xl w-full flex flex-col lg:items-center gap-5">
          ...
          <div class="flex flex-col gap-2">
            ...
            <div class="flex flex-wrap gap-2 sm:gap-3">
              ...
              <UBadge :color="coach?.data?.active === 'ENABLE' ? 'primary' : 'error'" variant="soft"
                :label="coach?.data?.active === 'ENABLE' ? 'فعال' : 'غیر فعال'" class="font-semibold" />
            </div>
          </div>
        </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-5 w-full">
        ...
        <div class="flex items-center gap-1">
          ...
          <span class="font-medium text-base mt-1">{{ gregorianToJalali(coach?.data?.createdAt) }}</span>
        </div>
        <div class="flex items-center gap-1">
        ...
        <div class="flex flex-col gap-5 w-full">
          <div class="flex max-sm:flex-col items-center gap-5 sm:gap-2 w-full">
            <BaseFormInput :required="false" :disable="isShow" v-model="state.fullName" label="نام و نام خانوادگی"
              name="fullName" type="text" placeholder="نام کامل مربی" class="w-full" />
          ...
          <div class="w-full">
            <BaseFormInput :required="false" :disable="isShow" v-model="state.certificates" label="مدرک و گواهینامه ها"
              name="certificates" type="text" placeholder="مدرک و گواهینامه ها مربیگری" class="w-full" />
          </div>
...
const { data: coach, refresh } = await useAsyncData('current-coach-profile', () => getCoachProfileService());
