  ...
  <section class="h-full w-full rounded-sm p-3 bg-muted flex flex-col gap-4">
    ...
    <BaseTabs v-model="active" :items="items" color="tertiary">
      <template #mali>
        ...
        <LazyWidgetModalPaymentAdminAdd v-model:open="modalStore.modals.paymentAdminAdd" @success="getPlanMaster" />
        <LazyWidgetModalPaymentAdminEdit v-model:open="modalStore.modals.paymentAdminEdit" @updated="handleUpdate" />
      </template>
    </BaseTabs>
  </section>
</template>
