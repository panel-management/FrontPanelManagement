...
const schema = v.object({
  ...
  price: v.union([
    ...
    v.pipe(
      v.string(),
      v.trim(),
      v.regex(/^\d+$/, 'قیمت باید فقط شامل عدد باشد')
    )
  ]),
  features: v.pipe(
    v.array(
...
<template>
  <UModal v-model:open="localOpen" title="ثبت پلن جدید" description="اطلاعات پلن را تکمیل کنید"
    :ui="{ footer: 'justify-between' }">
    <template #body>
      <UForm :schema="schema" :state="state" @submit.prevent="onSubmit">
        <div class="flex flex-col gap-5 w-full">
          <div class="w-full">
            <BaseFormInput v-model="state.name" label="نام پلن" name="name" type="text"
              placeholder="مثال: پلن رایگان , پلن پولی" required class="w-full" />
          </div>
          <div class="w-full">
          ...
          <div class="w-full flex flex-col">
            <div class="flex justify-between items-center">
              <label
                class="text-base font-medium after:text-red-500 after:content-['*'] after:text-sm after:pr-1">ویژگی‌ها</label>
              <UButton type="button" color="neutral" variant="solid" size="sm" label="افزودن ویژگی"
                @click="addItem()" />
            </div>
            <div v-for="(item, index) in state.features" :key="index" class="flex items-center gap-2 pt-2">
              <UFormField :name="`features.${index}`" class="w-full" :required="true">
                <UInput v-model="state.features[index]" type="text" class="w-full" color="neutral" size="lg" />
              </UFormField>
              <UButton v-if="state.features.length > 1" color="error" variant="soft"
                icon="material-symbols:delete-forever" @click="removeItem(index)" />
            </div>
          </div>
          <div class="w-full">
            <BaseFormSelect required :disable="false" v-model="state.type" :items="itemsSelect" name="type"
              placeholder="پلن مورد نظر را انتخاب کنید" label="انتخاب پلن" />
          </div>
          <div class="w-full">
