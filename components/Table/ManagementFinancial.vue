<template>
  <div class="w-full p-5 rounded-xl bg-white flex flex-col gap-3">
    <div class="w-full flex max-md:flex-col gap-3">
      <div class="flex max-sm:flex-col gap-3">
        ...
        <UButton
          ...
          @click="modalStore.toggleModal('equipmentAdd')"
          label="افزودن خرید تجهیزات"
          size="xl"
          color="neutral"
          variant="solid"
          class="flex- justify-center w-full md:w-fit"
    ...
    <div class="flex max-md:flex-col gap-3 w-full h-full">
      ...
      <BaseDropdownMenu
        :items="
          paymentMethodOptions.map((paymentMethod) => ({
            ...
            type: 'checkbox' as const,
            checked: selectedPaymentMethod.includes(paymentMethod),
            onUpdateChecked(checked: boolean) {
              if (checked) {
      ...
      <BaseDropdownMenu
        :items="
          typeTransactionOptions.map((typeTransaction) => ({
            label: transactionTypeLabels[typeTransaction],
            type: 'checkbox' as const,
            checked: selectedTypeTransaction.includes(typeTransaction),
            onUpdateChecked(checked: boolean) {
              if (checked) {
                selectedTypeTransaction.push(typeTransaction)
              } else {
                selectedTypeTransaction = selectedTypeTransaction.filter(
                  (b) => b !== typeTransaction
                )
              }
            },
      ...
      <BaseDropdownMenu
        :items="
          statusOptions.map((statusPayment) => ({
            label: paymentStatusLabels[statusPayment],
            type: 'checkbox' as const,
            checked: selectedStatus.includes(statusPayment),
            onUpdateChecked(checked: boolean) {
              if (checked) {
  ...
  <LazyWidgetModalEquipmentAdd
    v-model:open="modalStore.modals.equipmentAdd"
    @success="refreshPage"
  />
</template>
<script setup lang="ts">
  ...
  import {
    ...
    getHistoryTransactionMasterService,
  ...
  const paymentMethodOptions = ref([
    ...
    PaymentMethodStatus.CARD,
    PaymentMethodStatus.ONLINE,
  ])
  ...
  const filteredData = computed(() => {
    return formData.value.filter((row) => {
      const paymentMethodMatch =
        ...
        selectedPaymentMethod.value.includes(row.paymentMethod)
      const statusMatch =
        selectedStatus.value.length === 0 || selectedStatus.value.includes(row.status)
      const typeTransactionMatch =
        selectedTypeTransaction.value.length === 0 ||
        selectedTypeTransaction.value.includes(row.type)
    ...
    },
    {
      ...
      cell: ({ row }) =>
        h(
          'span',
          { class: 'font-medium' },
          `${Number(row.getValue('amount')).toLocaleString('fa-IR')} تومان`
        ),
    },
    {
      ...
      cell: ({ row }) => {
        return h('div', { class: 'flex gap-2' }, [
          h(UButton, {
            ...
            onClick() {
              showConfirmDialog(
                `آیا اطمینان دارید که هنرجو ${row.original.student.fullName} پرداخت انجام داده؟`,
                () => {
          ...
          h(UButton, {
            ...
            onClick() {
              showSendMessage(
                `آیا تأیید می‌کنید که هنرجو ${row.original.student.fullName} هنوز پرداختی انجام نداده است؟`,
                (desc) => {
