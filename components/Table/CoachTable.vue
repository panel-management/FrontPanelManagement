  ...
  <div class="flex-1 divide-y divide-accented w-full">
    <div class="flex max-lg:flex-col items-center gap-3 overflow-x-auto py-2">
      <UInput :model-value="(table?.tableApi?.getColumn('fullName')?.getFilterValue() as string)"
        ...
        class="w-full xl:w-1/3" placeholder="جستجو بر اساس نام مربی" size="xl" variant="outline" icon="i-lucide-search"
        color="primary" />
    ...
    <UTable ref="table" :data="filteredData" :loading="loadingModel" loading-color="neutral" :columns="columns"
...
const filteredData = computed(() => {
  return props.items.filter(row => selectedStatusAccount.value.length === 0 || selectedStatusAccount.value.includes(row.active))
})
