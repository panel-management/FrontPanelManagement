<template>
  <div class="w-full p-5 rounded-xl bg-white flex flex-col gap-3">
    <div class="w-full flex max-md:flex-col gap-3">
      ...
      />
      <UInput
        :model-value="table?.tableApi?.getColumn('phoneNumber')?.getFilterValue() as string"
        @update:model-value="table?.tableApi?.getColumn('phoneNumber')?.setFilterValue($event)"
        type="search"
        class="w-full"
        placeholder="جستجو بر اساس شماره تماس استاد"
        size="xl"
        variant="outline"
        icon="i-lucide-search"
        color="primary"
      ...
      <BaseDropdownMenu
        :items="
          statusOptions.map((statusPayment) => ({
            label: subscriptionStatusLabels[statusPayment],
            type: 'checkbox' as const,
...
<script setup lang="ts">
  ...
  } from '~/models/Payments/Subscriptions/SubscriptionListData'
  import {
    confirmSubscriptionsMasterService,
    getSubscriptionsPendingMasterService,
  ...
  const filteredData = computed(() => {
    return formData.value.filter(
      (row) => !selectedStatus.value.length || selectedStatus.value.includes(row.status)
    )
  })
      ...
      cell: ({ row }) => {
        return h('div', { class: 'flex gap-2' }, [
          h(UButton, {
            ...
            onClick: () => {
              showConfirmDialog(
                `آیا از تایید پرداخت ${row.original.master.fullName} اطمینان دارید؟`,
                () => {
                  confirmSubscription(row.original.id, {
                    status: statusConfirmSubscriptionsMaster.CONFIRMED,
                  })
                }
              )
            },
          ...
          h(UButton, {
            ...
            onClick: () => {
              showSendMessage(`رد کردن پرداخت ${row.original.master.fullName}`, (desc) => {
                confirmSubscription(row.original.id, {
                  status: statusConfirmSubscriptionsMaster.REJECTED,
                  adminNotes: desc,
                })
              })
            },
